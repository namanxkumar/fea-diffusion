# Largely based on https://github.com/lucidrains/denoising-diffusion-pytorch
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        return (
            F.normalize(x, dim=self.dim_to_normalize)
            * (x.shape[self.dim_to_normalize] ** 0.5)
            * self.g
        )
...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, positions: Tensor):
        ...
        embedding = torch.exp(
            torch.arange((self.embedding_dim // 2), device=device) * -embedding
        )
...
class LinearAttention(nn.Module):
    def __init__(
        ...
        self.memory_key_value = nn.Parameter(
            torch.randn(2, num_heads, head_dim, num_memory_key_value)
        )  # 2 for keys and values, num_mem_kv is the number of memory slots
        self.to_query_key_value = nn.Conv2d(input_dim, hidden_dim * 3, 1, bias=False)

        self.to_output = nn.Sequential(
            nn.Conv2d(hidden_dim, input_dim, 1), RMSNorm(input_dim)
        )
    ...
    def forward(self, x: Tensor):
        ...
        query_key_value: Tensor = cast(Tensor, self.to_query_key_value(x)).chunk(
            3, dim=1
        )  # split into q, k, v each of dimension [b, hidden_dim, h, w]

        query, key, value = map(
            lambda t: rearrange(t, "b (h c) x y -> b h c (x y)", h=self.num_heads),
            query_key_value,
        )  # split into heads, c is dim_head

        memory_key, memory_value = map(
            lambda t: repeat(t, "h c n -> b h c n", b=batch_size), self.memory_key_value
        )  # repeat memory keys and values across batch dimension
        key, value = map(
            partial(torch.cat, dim=-1), ((memory_key, key), (memory_value, value))
        )  # concat memory keys and values with the keys and values from the input, shape is b, heads, dim_head, num_mem_kv + w * h
        ...
        context = torch.einsum(
            "b h d n, b h e n -> b h d e", key, value
        )  # context between memory keys and values, shape is b, heads, dim_head, dim_head. d, e are dim_head

        output = torch.einsum(
            "b h d e, b h d n -> b h e n", context, query
        )  # output is query multiplied by the context, shape is b, heads, dim_head, w * h
        ...
        output = rearrange(
            ...
            x=image_height,
            y=image_width,
        )  # concat heads and dim_head dimensions
...
class FullAttention(nn.Module):
    def __init__(
        ...
        self.memory_key_value = nn.Parameter(
            torch.randn(2, num_heads, num_memory_key_value, head_dim)
        )
        self.to_query_key_value = nn.Conv2d(input_dim, hidden_dim * 3, 1, bias=False)
    ...
    def forward(self, x: Tensor):
        ...
        query, key, value = map(
            lambda t: rearrange(t, "b (h c) x y -> b h (x y) c", h=self.num_heads),
            query_key_value,
        )

        memory_key, memory_value = map(
            lambda t: repeat(t, "h n d -> b h n d", b=batch_size), self.memory_key_value
        )
        key, value = map(
            partial(torch.cat, dim=-2), ((memory_key, key), (memory_value, value))
        )

        output = self.attend(query, key, value)

        output = rearrange(
            output, "b h (x y) d -> b (h d) x y", x=image_height, y=image_width
        )
    ...
    def __init__(
        ...
        for index, (
            ...
            self.down_layers.append(
                nn.ModuleList(
                    [
                        resnet_module(input_dim, input_dim),
                        resnet_module(input_dim, input_dim),
                        attention_module(
                            input_dim,
        ...
        self.middle_block_1 = resnet_module(middle_dim, middle_dim)
        ...
        self.middle_block_2 = resnet_module(middle_dim, middle_dim)
        ...
        for index, (
            ...
            self.up_layers.append(
                nn.ModuleList(
                    [
                        resnet_module(output_dim + input_dim, output_dim),
                        resnet_module(output_dim + input_dim, output_dim),
                        attention_module(
                            output_dim,
                            num_heads=num_attention_heads,
